import { useCallback, useEffect, useRef, useState } from "react"
import { useNavigate, useParams } from "react-router-dom"
import { useQuery } from "@tanstack/react-query"
import { FormProvider, useForm } from "react-hook-form"
import { PlusCircle, SlidersHorizontal } from "lucide-react"
import Skeleton from "react-loading-skeleton"
import _ from "lodash"

// Services
import { getVacancies } from "../../services/vacancyService"

// Custom hooks
import { useMedia } from "../../hooks/useMedia"

// Components
import JobCard from "./components/JobCard"
import Search from "./components/Search"
import Menu from "../../components/Menu"
import Filters from "./components/Filters"
import Pagination from "../../components/Pagination"
import Button from "../../components/Button"
import DialogFilters from "./components/DialogFilters"

// Styles
import "./index.scss"
import "react-loading-skeleton/dist/skeleton.css"

const LIMIT = 20

function SkeletonJobCard({ isLast }) {
  function returnClassName() {
    let className = "skeleton-job-card"
    if (isLast) className += " skeleton-job-card--last"

    return className
  }

  return (
    <div className={returnClassName()}>
      <header>
        <div className="skeleton-job-card__inner-header-container">
          <Skeleton count={1} style={{ width: "60%", height: 30 }} />
          <div>
            <div>
              <Skeleton count={1} style={{ width: 60, height: 20 }} />
            </div>
            <div>
              <Skeleton count={1} style={{ width: 60, height: 20 }} />
            </div>
          </div>
        </div>
        <div>
          <Skeleton count={1} style={{ width: 50, height: 20 }} />
        </div>
      </header>
      <main>
        <Skeleton count={1} style={{ width: "100%", height: 140 }} />
      </main>
      <footer>
        <div>
          <Skeleton count={1} style={{ width: "60%", height: 25 }} />
        </div>
        <div>
          <Skeleton count={1} style={{ width: 140, height: 40 }} />
        </div>
      </footer>
    </div>
  )
}

function SkeletonBody() {
  return (
    <div className="jobs__skeleton-body">
      <header>
        <div>
          <Skeleton count={1} style={{ width: 100, height: 35 }} />
        </div>
      </header>
      <section>
        <SkeletonJobCard />
        <SkeletonJobCard />
        <SkeletonJobCard isLast />
      </section>
    </div>
  )
}

function Jobs() {
  const [filters, setFilters] = useState({})
  const [showFilters, setShowFilters] = useState(false)
  const isMobile = useMedia("(max-width: 960px)")
  const containerRef = useRef(null)

  const { page = 1, keyword = "" } = useParams()
  const navigate = useNavigate()
  const methods = useForm({
    defaultValues: {
      typeLocation: {
        onsite: false,
        remote: false,
        hybrid: false,
      },
      contractType: {
        clt: false,
        pj: false,
        internship: false,
      },
      employmentType: "",
      occupationArea: "",
    },
  })
  const { isLoading, data, error } = useQuery({
    queryKey: ["vacancies", page, filters, keyword],
    queryFn: () => fetchVacancies(),
    staleTime: 1000 * 60 * 10,
    refetchOnWindowFocus: true,
    keepPreviousData: true,
    retry: false,
  })

  const { watch } = methods

  const changePageNumber = useCallback(
    (pageNumber) => {
      if (keyword) {
        navigate(`/vagas/todas-vagas/${pageNumber}/${keyword}`)
        return
      }

      navigate(`/vagas/todas-vagas/${pageNumber}`)

      if (containerRef?.current) {
        containerRef.current.scrollTo({
          top: 0,
          behavior: "smooth",
        })
      }
    },
    [keyword, navigate],
  )

  useEffect(() => {
    const subscription = watch((value) => {
      setFilters(value)
      changePageNumber(1)
    })
    return () => subscription.unsubscribe()
  }, [watch, changePageNumber])

  async function fetchVacancies() {
    try {
      let newFilters = { ...filters }
      if (!_.isEmpty(newFilters)) {
        const typeLocationKeys = Object.keys(newFilters["typeLocation"])
        const contractTypeKeys = Object.keys(newFilters["contractType"])
        newFilters["typeLocation"] = typeLocationKeys.filter(
          (key) => newFilters["typeLocation"][key],
        )
        newFilters["contractType"] = contractTypeKeys.filter(
          (key) => newFilters["contractType"][key],
        )
      }

      const data = await getVacancies({ page, filters: newFilters, keyword })
      return data
    } catch (error) {
      console.error("Error fetching vacancies: ", error)
      return []
    }
  }

  function handleNavigate() {
    navigate("/vagas/criar-vaga")
  }

  const handleChangeKeyword = useCallback(
    (value) => {
      if (keyword === value) return
      navigate(`/vagas/todas-vagas/${1}/${value}`)
    },
    [navigate, keyword],
  )

  return (
    <div className="jobs">
      <div className="jobs-container">
        <Menu />
        <section className="jobs__main" ref={containerRef}>
          <div className="jobs__container-search-filters">
            <Search
              placeholder="Pesquise por vagas"
              initialValue={keyword}
              onChange={handleChangeKeyword}
            />
            {isMobile && (
              <div className="jobs__mobile-buttons">
                <Button
                  styleType="contained"
                  style={{
                    gap: "8px",
                    width: "fit-content",
                  }}
                  rightIcon={<PlusCircle size={18} />}
                  onClick={handleNavigate}
                >
                  Cadastrar vaga
                </Button>
                <Button
                  styleType="outlined"
                  rightIcon={<SlidersHorizontal />}
                  style={{ width: "fit-content", gap: "8px" }}
                  onClick={() => setShowFilters(true)}
                >
                  Filtros
                </Button>
              </div>
            )}
          </div>

          <section className="jobs__job-card-container">
            {isLoading && <SkeletonBody />}
            {!isLoading && error && <p>Erro ao carregar vagas</p>}
            {!isLoading && (
              <>
                <header>
                  <h1>Resultados ({data?.totalVacancies ?? 0})</h1>
                </header>
                <section className="jobs__cards">
                  {_.isEmpty(data?.vacancies) && !error && (
                    <p>Nenhuma vaga encontrada</p>
                  )}
                  {!_.isEmpty(data?.vacancies) &&
                    data.vacancies.map((job, index) => (
                      <div key={job._id}>
                        <JobCard job={job} />
                        {index < data.vacancies.length - 1 && (
                          <div className="jobs__divider" />
                        )}
                      </div>
                    ))}
                </section>
              </>
            )}
          </section>
          <footer>
            {!isLoading && !error && (
              <Pagination
                currentPage={page}
                onClickAction={changePageNumber}
                totalPages={
                  data?.totalVacancies
                    ? Math.ceil(data?.totalVacancies / LIMIT)
                    : 1
                }
              />
            )}
          </footer>
        </section>
        <FormProvider {...methods}>
          <DialogFilters
            open={showFilters}
            onNavigate={handleNavigate}
            onClose={() => setShowFilters(false)}
          />
          {!isMobile && (
            <Filters onNavigate={handleNavigate} hasVacancyButton />
          )}
        </FormProvider>
      </div>
    </div>
  )
}

export default Jobs
